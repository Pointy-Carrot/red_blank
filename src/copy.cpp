...
//Vars for color sort and socring
int forw  = 1;
bool fort = true;
bool runcolor = false;
//Default drivetrain
...
//runs double park last seconds of match
void runTime(){
    pros::Task unctask3([&]() {
...
//Functions for direction of the scoring meach
void forwSet(int i){
    forw = i;
}
...
//????? 
    ...
    //placehold task i dont think this is in actual code mayb i added for notebook
    pros::Task task2([&]() {
        while(true){
...
void opcontrol() {
    ...
    //CP is checker for if timer has run yet (boolean but an int)
    cp = 0;
	...
	while (true) {
        ...
        //Socring mech and intake controls
